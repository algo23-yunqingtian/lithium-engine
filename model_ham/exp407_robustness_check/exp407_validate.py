...
if maxcorr > 0.7:
    ...
    rep.append(f"- ✅ 共线性可控(最大相关{maxcorr:+.2f}≤0.7), 等权线性方案可用")
rep.append("")
...
rep.append("5. **回撤改善非全区间稳健**: 子样本检验显示回撤压缩效果部分存在, 非全行情普适——不可外推至未来")
rep.append("6. **本策略为长周期(20日持有)信号, 不适用于日内1分钟交易风格**\n")

rep.append("## 6. 局限性声明\n")
rep.append(f"- 样本量{len(F)}行, 约{len(F)//(TRAIN_WINDOW)}滚动窗口, 统计效力有限")
