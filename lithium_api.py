...
def init_minute_tables():
    ...
    conn.executescript("""
        ...
        CREATE INDEX IF NOT EXISTS idx_logic_scores_date ON logic_scores(date);
        CREATE INDEX IF NOT EXISTS idx_logic_scores_logic ON logic_scores(logic_name);
...
def api_battlefield():
    ...
    history_rows = conn.execute("""
        ...
        FROM agent_history ORDER BY date DESC, agent_id LIMIT 500
    """).fetchall()
...
@lithium_api.route('/logic-scores', methods=['GET'])
def api_logic_scores():
    """返回7维逻辑评分时序数据：供给风险/需求下滑/库存/政策/成本/情绪/基差"""
    days = int(request.args.get('days', 30))
    conn = get_db()

    # 从 logic_scores 表读取（后端计算后写入，只有 date/logic_name/avg/min/max/agent_count）
    rows = [dict(r) for r in conn.execute("""
        SELECT * FROM logic_scores
        WHERE date >= date('now', ?)
        ORDER BY date, logic_name
    ...
    series_data = {}
    for ln, label in LOGIC_NAMES.items():
        # 取该逻辑维度所有日期数据
        date_vals = {r['date']: r['avg_score'] for r in rows if r['logic_name'] == ln}
        series_data[ln] = {
            'name': label,
            'data': [{'date': d, 'value': date_vals.get(d, 0)} for d in dates]
        }
...
# ─── Agent Psychology API ──────────────────────────────────────────

# 主体中文名 + 主题色（暗色系，供色带/虚线/图例区分主体，设计文档第9节）
AGENT_PSY_NAMES = {
    'smelter': '冶炼厂',
    'merchant': '贸易商',
    'institution': '机构',
...
AGENT_PSY_STATE_COLORS = {
    '积极采购': '#3fb950', '观望': '#d29922', '恐慌撤退': '#f0883e',
...
@lithium_api.route('/agent-psychology', methods=['GET'])
def api_agent_psychology():
    """返回7主体心理状态 + 心理价位带时序（anchor/zone/score/trigger）"""
    days = int(request.args.get('days', 30))
    conn = get_db()

    rows = [dict(r) for r in conn.execute("""
        SELECT * FROM agent_psychology
        WHERE date >= date('now', ?)
        ORDER BY date, agent_id
    """, (f'-{days} days',)).fetchall()]
    ...
    agents = {}
    latest = {}
    for agent_id in AGENT_PSY_NAMES:
        adata = [r for r in rows if r['agent_id'] == agent_id]
        data = [{
            'date': r['date'],
            'state': r['state'],
            'state_color': r['state_color'],
            'anchor': int(round(r['anchor_price'])) if r['anchor_price'] is not None else None,
            'zone_low': int(round(r['buy_zone_low'])) if r['buy_zone_low'] is not None else None,
            'zone_high': int(round(r['buy_zone_high'])) if r['buy_zone_high'] is not None else None,
            'score': int(round(r['score'])) if r['score'] is not None else None,
            'trigger': r['trigger_desc'],
        } for r in adata]
        agents[agent_id] = {
            'name': AGENT_PSY_NAMES[agent_id],
            'color': AGENT_PSY_COLORS[agent_id],
            'data': data,
        }
        if data:
            latest[agent_id] = data[-1]

    return jsonify({
        'dates': dates,
        'agents': agents,
        'state_colors': AGENT_PSY_STATE_COLORS,
        'latest': latest,
        'latest_date': dates[-1] if dates else None,
    })
...
@lithium_api.route('/agent-interactions', methods=['GET'])
def api_agent_interactions():
    """返回agent间互动矩阵：冲突/跟随关系"""
    days = int(request.args.get('days', 30))
    conn = get_db()
    ...
    rows = [dict(r) for r in conn.execute("""
        ...
        WHERE date >= date('now', ?)
        ORDER BY date, from_agent, to_agent
    """, (f'-{days} days',)).fetchall()]
    ...
    all_agents = set()
    for r in rows:
        all_agents.add(r['from_agent'])
        all_agents.add(r['to_agent'])
    ...
    for r in rows:
        links.append({
            'source': r['from_agent'],
            'target': r['to_agent'],
            'type': r['influence_type'],
            'magnitude': round(r['magnitude'], 1),
            'date': r['date']
        })
    ...
    return jsonify({
        ...
        'signal_types': {
            'signal_resonance': sum(1 for r in rows if r['influence_type'] == 'signal_resonance'),
            'signal_conflict': sum(1 for r in rows if r['influence_type'] == 'signal_conflict'),
        },
    })


@lithium_api.route('/market-signals', methods=['GET'])
def api_market_signals():
    """返回升贴水/库存/产量等市场信号时序 + 每日阵营结构 (信号驱动互动)"""
    days = int(request.args.get('days', 30))
    conn = get_db()

    # 信号时序
    rows = [dict(r) for r in conn.execute("""
        SELECT date, signal, value, change, strength, direction, detail
        FROM market_signals
        WHERE date >= date('now', ?)
    ...
    by_date = {}
    for r in rows:
        by_date.setdefault(r['date'], {})[r['signal']] = r
    ...
    for d in dates:
        ...
        series.append({
            ...
            'inventory': sig.get('inventory'),
            'output': sig.get('output'),
            'price_momentum': sig.get('price_momentum'),
            'monthly_output': sig.get('monthly_output'),
...
@lithium_api.route('/lc-spreads', methods=['GET'])
def api_lc_spreads():
    """月差分析 API：合约间价差 + 持仓联动 + 正反套信号
    
    Query params:
      contracts: 逗号分隔合约列表，如 'LC2609,LC2611,LC2701'，默认 'LC2609,LC2611,LC2612,LC2701'
      include_position: 1=包含跨合约持仓数据（来自 lc_position.db）
    """
    import collections
    
    contracts_param = request.args.get('contracts', 'LC2609,LC2611,LC2612,LC2701')
    include_position = request.args.get('include_position', '1') == '1'
    contracts = [c.strip() for c in contracts_param.split(',') if c.strip()]
    ...
    for c in contracts:
        rows = conn.execute(
            "SELECT date, open, high, low, close, volume, position, settle FROM contract_prices WHERE contract=? ORDER BY date",
            (c,)
        ).fetchall()
        if rows:
            contract_prices[c] = [dict(r) for r in rows]
    conn.close()
    ...
    for i in range(len(contracts) - 1):
        near = contracts[i]
        far = contracts[i + 1]
        if near not in contract_prices or far not in contract_prices:
            continue
        
        # 用 settle 价格（收盘价也行）
        near_dates = {r['date']: r['settle'] for r in contract_prices[near]}
        far_dates = {r['date']: r['settle'] for r in contract_prices[far]}
        common_dates = sorted(set(near_dates.keys()) & set(far_dates.keys()))
        
        spread_data = []
        for d in common_dates:
            spread = near_dates[d] - far_dates[d]
            spread_data.append({
                'date': d,
                'near': near_dates[d],
                'far': far_dates[d],
                'spread': spread,
                'spread_pct': round((spread / far_dates[d]) * 10000, 1)  # 以 points 计
            })
    ...
    # 3. 从 lc_position.db 获取跨合约持仓数据（如果请求）
    position_data = {}
    if include_position:
        try:
            lc_db = '/home/ubuntu/lc_futures_data/data/lc_position.db'
            lc_conn = sqlite3.connect(lc_db)
            lc_conn.row_factory = sqlite3.Row
            for c in contracts:
                # DB 里存大写 (LC2609)，API 传入也是大写，直接匹配
                symbol = c.upper()
                ...
                if rows:
                    position_data[c] = [dict(r) for r in rows]
            lc_conn.close()
        except Exception:
            pass
    
    # 4. 从 spreads 表获取已有相邻月差
    conn2 = sqlite3.connect(DB_PATH)
    conn2.row_factory = sqlite3.Row
    existing_spreads = {}
    for sp in spread_pairs:
        key = f"{sp['near']}-{sp['far']}"
        rows = conn2.execute(
            "SELECT date, spread FROM spreads WHERE near_contract=? AND far_contract=?",
            (sp['near'], sp['far'])
        ).fetchall()
        if rows:
            existing_spreads[key] = {str(r['date']): r['spread'] for r in rows}
    conn2.close()
    
    # 5. 计算正反套信号
    # 正套：近月升水（spread > 0），近月持仓增 + 远月持仓减 = 正套开仓
    # 反套：近月贴水（spread < 0），近月持仓减 + 远月持仓增 = 反套开仓
    signals = {}
    for sp in spread_pairs:
        near = sp['near']
        far = sp['far']
        if near not in position_data or far not in position_data:
            continue
        
        near_by_date = {r['date']: r for r in position_data[near]}
        far_by_date = {r['date']: r for r in position_data[far]}
        common_dates = sorted(set(near_by_date.keys()) & set(far_by_date.keys()))
        
        daily_signals = []
        for d in common_dates:
            n = near_by_date[d]
            f = far_by_date[d]
            n_chg = n.get('long_oi', 0) - n.get('short_oi', 0)  # 净增方向
            f_chg = f.get('long_oi', 0) - f.get('short_oi', 0)
            
            spread = near['settle'] if hasattr(near, 'settle') else 0
            
            # 简化信号：看净持仓变化方向
            if n['long_oi'] - n['short_oi'] > f['long_oi'] - f['short_oi']:
                direction = 'positive'  # 正套偏多
            elif n['long_oi'] - n['short_oi'] < f['long_oi'] - f['short_oi']:
                direction = 'negative'  # 反套偏多
            else:
                direction = 'neutral'
        ...
        signals[f"{near}-{far}"] = daily_signals
    ...
    result = {
        ...
        'signals': signals,
        'date_ranges': {c: (min(r['date'] for r in v), max(r['date'] for r in v)) 
                        for c, v in contract_prices.items() if v}
    }
